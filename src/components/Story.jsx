...
const PARTNER_NAME = 'Shriram Bioseed'
    ...
    <section id="story" ref={ref} className="relative bg-sage py-11 md:py-14 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-[1.35fr_1fr] gap-12 items-start">
          <div>
            <div className="reveal eyebrow eyebrow-rule text-green-700">{t('story.eyebrow')}</div>
            ...
            {/* Our Strength */}
            <div className="reveal mt-8 text-[11px] uppercase tracking-[0.24em] font-bold text-ink-mute" style={{ fontFamily: 'var(--font-sans)', '--reveal-delay': '140ms' }}>{t('story.strengthLabel')}</div>
              ...
              {STATS.map((s, i) => (
                <div key={s.label} className="reveal card card-hover px-6 py-6" style={{ '--reveal-delay': `${i * 80}ms` }}>
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-amber-deep text-sm font-medium italic" style={{ fontFamily: 'var(--font-serif)' }}>{t(`stats.${s.sub}`)}</span>
                  </div>
                  <div className="mt-2.5 text-[11px] uppercase tracking-[0.16em] font-semibold text-ink-mute leading-relaxed" style={{ fontFamily: 'var(--font-sans)' }}>{t(`stats.${s.sub}Label`)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="reveal relative h-[440px] sm:h-[520px] hidden sm:block" style={{ '--reveal-delay': '200ms' }}>
            <div data-parallax="0.16" className="absolute top-0 right-6 w-[62%]">
              <div className="photo-frame relative overflow-hidden" style={{ '--tilt': '2.5deg', animation: 'float-idle 7s ease-in-out infinite alternate' }}>
                ...
                <div className="absolute inset-x-0 bottom-0 px-3 pt-9 pb-2.5 text-center text-[10px] uppercase tracking-[0.18em] font-bold text-white" style={{ background: 'linear-gradient(to top, rgba(20,47,27,0.72), transparent)', fontFamily: 'var(--font-sans)' }}>{t('story.photoGrainCheck')}</div>
              </div>
            </div>
            <div data-parallax="-0.13" className="absolute bottom-0 left-0 w-[58%]">
              <div className="photo-frame relative overflow-hidden" style={{ '--tilt': '-3deg', animation: 'float-idle 8.5s ease-in-out 0.9s infinite alternate' }}>
                ...
                <div className="absolute inset-x-0 bottom-0 px-3 pt-9 pb-2.5 text-center text-[10px] uppercase tracking-[0.18em] font-bold text-white" style={{ background: 'linear-gradient(to top, rgba(20,47,27,0.72), transparent)', fontFamily: 'var(--font-sans)' }}>{t('story.photoHarvestDay')}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-14 reveal" style={{ '--reveal-delay': '120ms' }}>
          <div className="relative">
            ...
            <div className="flex flex-col md:flex-row md:justify-between gap-10 md:gap-4">
              {TIMELINE.map((tl, i) => (
                <div key={tl.year} className="relative flex md:flex-col items-start gap-5 md:gap-0 md:flex-1 group">
                  ...
                  <div className="md:mt-6 -mt-1">
                    <div className="text-3xl sm:text-4xl font-light text-green-800 tracking-tight" style={{ fontFamily: 'var(--font-serif)' }}>{tl.year}</div>
                    <div className="text-amber-deep text-[10.5px] uppercase tracking-[0.2em] font-bold mt-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t(`story.timeline.${tl.year}.label`)}</div>
                    <div className="mt-1.5 text-ink-soft text-[13px] leading-relaxed max-w-[190px]">{t(`story.timeline.${tl.year}.note`)}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-10 reveal">
          <div className="flex flex-wrap items-end justify-between gap-3 mb-7">
            ...
            <div className="text-[11px] uppercase tracking-[0.2em] text-ink-mute font-bold" style={{ fontFamily: 'var(--font-sans)' }}>{t('story.partnersMeta')}</div>
          </div>
            ...
            <div className="marquee-track flex gap-4 w-max items-stretch">
              {[0, 1].map((dup) => (
                <div key={dup} className="flex gap-4 items-stretch">
                  ...
                  {[PARTNER_NAME, t('story.othersLabel')].map((n) => (
