...
// through to their varieties; the rest carry a Coming Soon tag. Names/subs
// come from the crops.<key> translation namespace, keyed by these ids.
const FIELD = [
  { key: 'hybrid-maize', img: '/images/photos/field-maize-rows.jpg', crop: 'maize' },
  { key: 'hybrid-paddy', img: '/images/photos/paddy-panicle.jpg', crop: 'paddy' },
  { key: 'research-paddy', img: '/images/photos/paddy-grain-check.jpg', crop: 'paddy' },
  { key: 'wheat', img: '/images/photos/crops/wheat.jpg' },
  { key: 'mustard', img: '/images/photos/crops/mustard.jpg' },
  { key: 'fodder-bajra', img: '/images/photos/crops/bajra.jpg' },
  { key: 'fodder-sorghum', img: '/images/photos/crops/sorghum.jpg' },
]

const VEGETABLE = [
  { key: 'sweet-corn', img: '/images/photos/sweetcorn-cobs.jpg', crop: 'sweetcorn' },
  { key: 'okra', img: '/images/photos/crops/okra.jpg' },
  { key: 'cowpea', img: '/images/photos/crops/cowpea.jpg' },
  { key: 'radish', img: '/images/photos/crops/radish.jpg' },
  { key: 'cluster-beans', img: '/images/photos/crops/clusterbeans.jpg' },
  { key: 'cucumber', img: '/images/photos/crops/cucumber.jpg' },
  { key: 'beans', img: '/images/photos/crops/beans.jpg' },
]

function Tile({ c, onCrop, t }) {
  const available = !!c.crop
  const name = t(`portfolio.crops.${c.key}.name`)
  const sub = t(`portfolio.crops.${c.key}.sub`)
  const inner = (
    <>
      <img src={c.img} alt={`${name} crop`} loading="lazy" className={`absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.08] ${available ? '' : 'saturate-[0.92]'}`} />
      ...
      >
        {available ? t('portfolio.available') : t('portfolio.comingSoon')}
      </span>
      <div className="absolute inset-x-0 bottom-0 p-4">
        <div className="text-white text-[17px] font-medium leading-tight" style={{ fontFamily: 'var(--font-serif)' }}>{name}</div>
        <div className="mt-0.5 flex items-center justify-between">
          <span className="text-white/72 text-[9.5px] uppercase tracking-[0.14em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>{sub}</span>
          {available && (
  ...
  return available ? (
    <button onClick={() => onCrop(c.crop)} aria-label={`Browse ${name}`} className={`${base} text-left cursor-pointer hover:shadow-[0_22px_44px_-16px_rgba(20,47,27,0.5)] transition-shadow duration-500`}>{inner}</button>
  ) : (
    <div className={base} title={`${name} · ${t('portfolio.comingSoon')}`}>{inner}</div>
  )
}
...
function River({ items, reverse, onCrop, label, t }) {
    ...
    <div>
      ...
      >
        <div className="flex gap-4 w-max py-2">
          {[...items, ...items].map((c, i) => (
            <Tile key={`${c.key}-${i}`} c={c} onCrop={onCrop} t={t} />
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <section id="coming" ref={ref} className="relative bg-sage pt-9 pb-10 md:pt-10 md:pb-12 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        <div className="reveal eyebrow eyebrow-rule text-green-700">{t('portfolio.eyebrow')}</div>
      ...
      <div className="relative mt-7 space-y-6">
        <River items={FIELD} reverse label={t('portfolio.fieldCropsLabel')} onCrop={onCrop} t={t} />
        <River items={VEGETABLE} label={t('portfolio.vegetableSeedsLabel')} onCrop={onCrop} t={t} />
      </div>
    </section>
  )
}
