...
// canonical English values are what gets stored/submitted regardless of the
// UI language — only the displayed option text changes per language
const ROLES = en.enquiry.roles
const CROPS = en.enquiry.crops
...
export default function Enquiry() {
  const ref = useReveal()
  const { t } = useLang()
  const roleLabels = t('enquiry.roles')
  const cropLabels = t('enquiry.crops')
    ...
    <section id="enquiry" ref={ref} className="relative bg-sage py-16 md:py-20">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-[1fr_1.3fr] gap-10 lg:gap-16 items-start">
          <div>
            <div className="reveal eyebrow eyebrow-rule text-green-700">{t('enquiry.eyebrow')}</div>
            ...
            <div className="reveal mt-7 space-y-3" style={{ '--reveal-delay': '200ms' }}>
              ...
              <a href={`tel:${CONTACT.customerCareRaw}`} className="flex items-center gap-3 text-green-800 font-semibold text-[15px] hover:text-green-700">
                ...
                {CONTACT.customerCare} · {t('common.customerCareLabel')}
              </a>
            </div>
          </div>
          ...
          <div className="reveal card p-7 md:p-9" style={{ '--reveal-delay': '160ms' }}>
            {status === 'done' ? (
              <div className="text-center py-10">
                ...
                <h3 className="mt-5 text-2xl text-green-950" style={{ fontFamily: 'var(--font-serif)' }}>{t('enquiry.thankYou', form.name.split(' ')[0] || t('enquiry.thankYouFallback'))}</h3>
                <p className="mt-2 text-ink-soft text-[14.5px]">{t('enquiry.successNote')}</p>
              ...
              <form onSubmit={onSubmit} noValidate>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="enq-name" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelName')}</label>
                    <input id="enq-name" className={inputCls} value={form.name} onChange={set('name')} placeholder={t('enquiry.placeholderName')} required />
                  </div>
                  <div>
                    <label htmlFor="enq-phone" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelPhone')}</label>
                    <input id="enq-phone" className={inputCls} value={form.phone} onChange={set('phone')} placeholder={t('enquiry.placeholderPhone')} inputMode="tel" required />
                  </div>
                  <div>
                    <label htmlFor="enq-role" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelRole')}</label>
                    <select id="enq-role" className={inputCls} value={form.role} onChange={set('role')}>
                      {ROLES.map((r, i) => <option key={r} value={r}>{roleLabels[i]}</option>)}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="enq-crop" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelCrop')}</label>
                    <select id="enq-crop" className={inputCls} value={form.crop} onChange={set('crop')}>
                      {CROPS.map((c, i) => <option key={c} value={c}>{cropLabels[i]}</option>)}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="enq-district" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelDistrict')}</label>
                    <input id="enq-district" className={inputCls} value={form.district} onChange={set('district')} placeholder={t('enquiry.placeholderDistrict')} />
                  </div>
                  <div>
                    <label htmlFor="enq-state" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelState')}</label>
                    <input id="enq-state" className={inputCls} value={form.state} onChange={set('state')} placeholder={t('enquiry.placeholderState')} />
                  </div>
                </div>
                <div className="mt-4">
                  <label htmlFor="enq-msg" className="block text-[11px] uppercase tracking-[0.16em] font-bold text-ink-soft mb-1.5" style={{ fontFamily: 'var(--font-sans)' }}>{t('enquiry.labelMessage')}</label>
                  <textarea id="enq-msg" className={`${inputCls} min-h-[88px] resize-y`} value={form.message} onChange={set('message')} placeholder={t('enquiry.placeholderMessage')} />
                </div>
                ...
                {status === 'error' && (
                  <p className="mt-4 text-[13px] text-red-700 bg-red-50 border border-red-200 rounded-lg px-4 py-2.5">
                    {t('enquiry.errorMessage', CONTACT.customerCare)}
                  </p>
                )}
                ...
                <button type="submit" disabled={status === 'sending'} className="btn-primary mt-6 w-full justify-center disabled:opacity-60 disabled:cursor-wait">
                  {status === 'sending' ? t('enquiry.submitSending') : t('enquiry.submitIdle')}
                </button>
                <p className="mt-3 text-center text-[11.5px] text-ink-mute">{t('enquiry.privacyNote')}</p>
              </form>
            )}
          </div>
        </div>
