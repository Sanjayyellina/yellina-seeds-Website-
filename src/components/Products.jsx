...
const TRUTHFUL_KEYS = {
  'Physical Purity': 'physicalPurity',
  Germination: 'germination',
  'Genetic Purity': 'geneticPurity',
  Moisture: 'moisture',
}

// truthful values are always "<pct>% min|max" in the data — swap in the
// translated unit word (min/max) while keeping the number as-is
function truthfulValue(t, value) {
  const m = value.match(/^(\d+)% (min|max)$/)
  return m ? `${m[1]}% ${t(`products.truthful.${m[2]}`)}` : value
}

// stats' `value` field can be a duration number (language-agnostic) or a
// short descriptive word (Kharif & Rabi / Early / Premium Long-Duration)
// that has its own per-product translation under products.items.<id>
const STAT_VALUE_KEYS = { Season: 'seasonValue', Maturity: 'maturityValue', Type: 'typeValue' }

function statValue(t, id, s) {
  const key = STAT_VALUE_KEYS[s.label]
  return key ? t(`products.items.${id}.${key}`) : s.value
}

function ProductCard({ p, onOpen, delay, accent, t }) {
  const name = t(`products.items.${p.id}.name`)
  const tagline = t(`products.items.${p.id}.tagline`)
  return (
    <button
      onClick={() => onOpen(p)}
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-baseline justify-between gap-2">
          <h3 className="text-[18px] text-green-950 font-medium leading-tight" style={{ fontFamily: 'var(--font-serif)' }}>{name}</h3>
        ...
        <p className="mt-1.5 text-[12px] leading-relaxed text-ink-soft line-clamp-2">{tagline}</p>

        {/* truthful-label facts, surfaced right on the tile instead of hidden behind a click */}
        <div className="mt-3 grid grid-cols-2 gap-1.5">
          {p.truthful.slice(0, 2).map((f) => (
            <div key={f.label} className="rounded-lg bg-sage px-2 py-1.5 text-center">
              <div className="text-[7.5px] uppercase tracking-[0.1em] text-green-700 font-bold" style={{ fontFamily: 'var(--font-sans)' }}>{t(`products.truthful.${TRUTHFUL_KEYS[f.label]}`)}</div>
              <div className="text-[11.5px] font-semibold text-green-950">{truthfulValue(t, f.value)}</div>
            </div>
          ))}
        </div>
...
function ProductModal({ p, onClose, onPrev, onNext, t }) {
  useEffect(() => {
    const onKey = (e) => {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center p-0 sm:p-6" role="dialog" aria-modal="true" aria-label={`${name} details`}>
      ...
      <div className="modal-panel relative glass-light w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl">
        ...
        <div className="grid md:grid-cols-[1fr_1.2fr]">
          ...
          <div className="relative min-h-[280px] md:min-h-full flex flex-col items-center justify-center p-8" style={{ background: 'radial-gradient(ellipse at 50% 30%, rgba(125,179,67,0.18), rgba(240,244,231,0.9) 75%)' }}>
            <img src={p.image} alt={`${name} pack design`} className="w-full max-w-[220px] object-contain" />
            <div className="mt-5 text-[9px] uppercase tracking-[0.2em] text-amber-deep font-bold" style={{ fontFamily: 'var(--font-sans)' }}>{t('products.packDesign')}</div>
          </div>

          {/* Details */}
          <div className="p-7 md:p-10">
            <div className="eyebrow text-green-700 !text-[10px]">{p.type}{p.code ? ` · ${p.code}` : ''}</div>
            <h3 className="mt-2 text-4xl text-green-950 font-medium" style={{ fontFamily: 'var(--font-serif)' }}>{name}</h3>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {t(`products.items.${p.id}.badges`).map((b) => (
            ...
            <div className="mt-5 rounded-2xl border border-line bg-white p-5">
              <div className="text-[10px] uppercase tracking-[0.22em] text-amber-deep font-bold mb-3.5" style={{ fontFamily: 'var(--font-sans)' }}>
                {t('products.truthfulLabelHeading')}
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2.5">
                {p.truthful.map((f) => (
                  <div key={f.label} className="flex flex-col">
                    <span className="text-[10px] text-ink-mute">{t(`products.truthful.${TRUTHFUL_KEYS[f.label]}`)}</span>
                    <b className="text-green-900 text-[13px] font-semibold">{truthfulValue(t, f.value)}</b>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="mt-7 grid grid-cols-2 gap-2.5">
                {p.stats.map((s) => (
                  <div key={s.label} className="rounded-xl bg-sage px-3 py-3.5 text-center">
                    <div className="text-[8.5px] uppercase tracking-[0.18em] text-green-700 font-bold" style={{ fontFamily: 'var(--font-sans)' }}>{t(`products.statLabels.${s.label.toLowerCase()}`)}</div>
                    <div className="mt-1 text-green-950 font-semibold text-[15px] leading-tight">
                      {statValue(t, p.id, s)}
                      {s.unit && <span className="block text-[10px] font-normal text-ink-mute mt-0.5">{t('products.daysUnit')}</span>}
                    </div>
                  </div>
                ))}
              </div>
    ...
    <section id="products" ref={ref} className="relative py-11 md:py-14 bg-bg overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <div className="reveal eyebrow eyebrow-rule text-green-700">{t('products.eyebrow')}</div>
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {t(`products.categories.${c.id}.label`)}
                </button>
              )
            })}
          </div>
        ...
        <div key={cat} className="mt-9">
          <div className="flex items-baseline justify-between flex-wrap gap-2">
            <p className="reveal is-visible text-ink-soft text-base leading-relaxed max-w-2xl">
              <span className="italic text-green-900 text-lg block mb-1.5" style={{ fontFamily: 'var(--font-serif)' }}>{t(`products.categories.${current.id}.heading`)}</span>
              {t(`products.categories.${current.id}.intro`) || null}
            </p>
            <span className="text-[11px] uppercase tracking-[0.16em] font-bold text-ink-mute" style={{ fontFamily: 'var(--font-sans)' }}>
              {t('products.varietyCount', items.length)}
            </span>
          </div>

          <div className="mt-7 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {items.map((p, i) => (
              <ProductCard key={p.id} p={p} onOpen={setActive} delay={i * 80} accent={CATEGORY_ACCENT[cat]} t={t} />
            ))}
          </div>
        </div>
      </div>
